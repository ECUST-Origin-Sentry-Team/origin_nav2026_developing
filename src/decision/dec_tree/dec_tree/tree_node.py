...
class GetDataFromYaml(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, name: str, yaml_name: str, node: Node):
        ...
        self.blackboard.__setattr__("our_color",self.node.get_parameter('self_color').get_parameter_value().string_value)
        self.node.get_logger().info(f"our_color: {self.node.get_parameter('self_color').get_parameter_value().string_value}")
    def update(self):
...
class Patrol(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if self.blackboard.dec_now != self.name or self.blackboard.is_chase:
            self.init_dec()
...
class CallIsAbleToChase(py_trees.behaviour.Behaviour):
    def __init__(self, name: str,node:Node, nav: BasicNavigator):
        ...
        self.wait_begin = False
    def update(self):
        self.able_to_chase_list = list(self.yaml.activate_chase_point.keys())
        ''' 
        如果:
            没有开启追击
            没有到点
        ...
        继续执行后面的树
        '''
        if not self.yaml.activate_chase or self.blackboard.dec_now not in self.able_to_chase_list or self.blackboard.ChaseCall.chase_ready == False :
            self.wait_begin = False
            self.wait_until = 0.0
            return py_trees.common.Status.FAILURE
        else:
            # 如果是从不满足追击到开始追击或者追击时间过了1秒，就发布新的点
            if self.wait_begin == False or self.wait_until < time.time(): 
                self.wait_until = time.time() + 1.0
                self.wait_begin = True
                self.blackboard.goal = {'x': self.blackboard.ChaseCall.chase_x,'y':self.blackboard.ChaseCall.chase_y}
...
class PublishChaseGoal(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        msg.max_distance_from_goal = self.yaml.activate_chase_point.get(self.blackboard.dec_now,0.0)
        self.publisher.publish(msg)
        self.node.get_logger().info(f"[tree] Published ChaseGoal: {msg.x}, {msg.y}, {msg.max_distance_from_goal}")
        return py_trees.common.Status.SUCCESS
