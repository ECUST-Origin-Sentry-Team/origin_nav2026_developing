...
from .tree_node import GetDataFromYaml,PubGoal,CheckNavState,Patrol,YawDec,PitchDec,UnpackReferee,OutpostAttackDec,ReachEnemyHeroPos,CallIsAbleToChase,PublishChaseGoal
...
def create_get_data(node,qos_profile,nav):
    ...
    save_Chase_data = py_trees_ros.subscribers.ToBlackboard(
    ...
    save_Chase_data.setup(node=node)
    

    get_data.add_children(
        [get_data_from_yaml,referee_list,check_nav_state,save_Chase_data]
    )
...
def create_dec(node,nav,qos_profile):
    ...
    def condition_go_to_outpost(patrol):
        ...
        go_to_outpost = patrol.blackboard.Referee.stage_remain_time < 400 and getattr(patrol.blackboard.Referee,patrol.their_color + '_outpost_hp') > 0
        if  go_to_outpost :
            return True
        return False
    ...
    dec_selector.add_children(
        [goto_home,goto_chase,goto_outpost,goto_last_stand,goto_catch_hero,goto_mid]
    )

    dec.add_children(
        [dec_selector,pitch,enemy_hero,outpost_attack_list,pub_chase,pub_goal]
    )
